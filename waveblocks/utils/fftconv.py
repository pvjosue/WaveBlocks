...
def fft_conv(
    ...
    # Check if the kernel is already in fourier space
    kernel_size = kernel.shape
    if torch.is_complex(kernel):
        kernel_fr = kernel
        kernel_size = signal_size
    else:
        kernel_padding = [
            pad
    ...
    # Perform fourier convolution -- FFT, matrix multiply, then IFFT
        ...
        # Pytorch does correlation, not convolution, so this is needed
        # # And in case of the need for correlation or OTF multiplication, it can be disabled
        if not invert:
            kernel_fr.imag *= -1
    ...
    crop_slices = [slice(0, output.size(0)), slice(0, output.size(1))] + [
        slice(0, (signal.size(i) - kernel_size[i] + 1), stride_[i - 2])
