% LFBuildLensletGridModel - builds a lenslet grid model from a white image
%
% Usage:
%     [LensletGridModel, GridCoords] = LFBuildLensletGridModel( WhiteImg, gridType, GridModelOptions, DebugDisplay )
%
% Inputs: 
...
function [LensletGridModel, GridCoords] = LFBuildLensletGridModel( WhiteImg, grid_type, GridModelOptions, DebugDisplay )
...
if (strcmp(grid_type, 'reg'))
    LensletGridModel = struct('HSpacing',XSpacing, 'VSpacing',XSpacing, 'HOffset',GridModelOptions.CropAmt, ...
    ...
    LensletGridModel.VMax = ceil( (size(WhiteImg,1)-GridModelOptions.CropAmt*2)/XSpacing );
else
...
GridCoords = LFBuildGrid( LensletGridModel, grid_type);
...
VSteps = round( (LensletGridModel.VOffset - NewVOffset) / LensletGridModel.VSpacing ); % should be a whole number
    ...
    if (strcmp(grid_type, 'hex'))
        ...
        NewHOffset = mod( LensletGridModel.HOffset, LensletGridModel.HSpacing );
